...
additive_expression
	...
	//	debug ("adding expressions: $1: " <<$1->symp);	
		if (typecheck($1->symp, $3->symp)) {
			$$ = new expr();
			$$->symp = gentemp(_INT);
...
equality_expression
	...
	| equality_expression EQ_OP relational_expression {
		if (typecheck ($1->symp, $3->symp) ) {
			...
			$$->truelist = makelist (nextinstr());
			$$->falselist = makelist (nextinstr()+1);
			emit (EQOP, $$->symp->name, $1->symp->name, $3->symp->name);
			emit (GOTOOP, "");
		}
		else cout << "Type Error"<< endl;
	}
...
type_qualifier_list_opt /* Ignored */
	: %empty
	| type_qualifier_list
...
selection_statement
	...
	| IF '(' expression ')' M statement N ELSE M statement {
		...
		lint temp = merge ($6->nextlist, $7->nextlist);
